...
noiseVol = nentry("noiseVo[style:knob]l", 1, 0, 8, 0.001) : si.smoo;
noiseCO = nentry("noiseCO[style:knob]", 100, 100, 2000, 0.1) : si.smoo;
...
*noiseGroup(vslider("volume", 0, 0, 1, 0.01)) * maxLevel
  :>_<:_,_
	with {
		...
		partial(j) = vgroup("[%k]BL%j", (os.pulsetrain(freq, duty): fi.lowpass(6, cutoff)) * gain * env : ef.echo(maxDur, dur, fb))
	...
	with {
		...
		k = j+10;
		env = en.ar(att, rel, t);
		att = nentry("[2]attack[style:knob]", 0.5, 0.001, 5, 0.001);
